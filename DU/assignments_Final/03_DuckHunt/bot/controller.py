...
class DartMasterController:
    """
    Central coordinator for the bot. 

    Main jobs:
    - fetch a frame from the Windows bridge
    - preprocess it
    - match templates across configured scales  
    - optionally update prediction history
    - choose the click point
    - decide whether clicking is allowed
    - send the click
    ...
    def __init__(self, cfg: BotConfig):
        ...
        self.matcher = TemplateMatcher(cfg.match.method, cfg.match.scales)  
    ...
    def run(self) -> None:
        ...
        # Print loaded template names and scales for startup verification.  
        print("Loaded templates:", [template.name for template in self.templates])
        print("Configured scales:", list(self.cfg.match.scales))  
    ...
    def _run_one_iteration(self) -> None:
        ...
        # use full-frame template matching, now across configured scales.  
        ...
        if self._should_click(best_match, click_point):
            ...
            print(
                ...
                f"size={best_match.matched_width}x{best_match.matched_height} " 
